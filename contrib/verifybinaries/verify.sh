...
###   bitcoinrushcore.org and bitcoinrush.org and compares them.
...
WORKINGDIR="/tmp/bitcoinrush_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://bitcoinrushcore.org"
HOST2="https://bitcoinrush.org"
BASEDIR="/bin/"
VERSIONPREFIX="bitcoinrush-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'bitcoinrush-',
...
if [ $? -ne 0 ]; then
   echo "bitcoinrush.org failed to provide signature file, but bitcoinrushcore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "bitcoinrush.org and bitcoinrushcore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Bitcoinrush Core binary release signing key installed?"
   fi
