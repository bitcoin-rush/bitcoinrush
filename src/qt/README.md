This directory contains the BitcoinrushQT graphical user interface (GUI). It uses the cross-platform framework [Qt](https://www1.qt.io/developers/).
...
### bitcoinrushgui.(h/cpp)

Represents the main window of the Bitcoinrush UI.
...
ClientModel is used by the main application `bitcoinrushgui` and several models like `peertablemodel`.
...
Used to process BIP21 and BIP70 (see https://github.com/bitcoinrush/bitcoinrush/pull/11622) payment URI / requests. Also handles URI based application switching (e.g. when following a bitcoinrush:... link from a browser).
...
* UI elements like BitcoinrushAmountField, which inherit from QWidget.
* `bitcoinrushstrings.cpp`: automatically generated
* `bitcoinrushunits.(h/cpp)`: RUSH / mRUSH / etc handling
* `callback.h`
...
* don't change `local/bitcoinrush_en.ts`; this happens [automatically](/doc/translation_process.md#writing-code-with-translations)
...
4. Enter "bitcoinrush-qt" as project name, enter src/qt as location
5. Leave the file selection as it is
6. Confirm the "summary page"
7. In the "Projects" tab select "Manage Kits..."
8. Select the default "Desktop" kit and select "Clang (x86 64bit in /usr/bin)" as compiler
9. Select LLDB as debugger (you might need to set the path to your installation)
10. Start debugging with Qt Creator (you might need to the executable to "bitcoinrush-qt" under "Run", which is where you can also add command line arguments)
