...
const char * const BITCOINRUSH_CONF_FILENAME = "bitcoinrush.conf";
const char * const BITCOINRUSH_PID_FILENAME = "bitcoinrushd.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "bitcoinrushd -foo=bar
...
#else
    const char* pszModule = "bitcoinrush";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Bitcoinrush
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Bitcoinrush
    // Mac: ~/Library/Application Support/Bitcoinrush
    // Unix: ~/.bitcoinrush
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Bitcoinrush";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Bitcoinrush";
#else
    // Unix
    return pathRet / ".bitcoinrush";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", BITCOINRUSH_CONF_FILENAME);
...
fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", BITCOINRUSH_PID_FILENAME)));
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Bitcoinrush Core") == std::string::npos) {
